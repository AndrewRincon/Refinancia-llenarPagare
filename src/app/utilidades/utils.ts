...
export function Uint8ArrayToStringBase64(data: Uint8Array): string {
    let binary = '';
 ...
 */
export function stringToUint(src: string): Uint8Array {
 ...
 */
export function getExtensionImageFromURI(base64: string): string {
 ...
 */
export function encryptString(strEncrypt, key) {
    return CryptoJS.AES.encrypt(strEncrypt, key).toString();
}
 ...
 */
export function getHexColorFromRgba(color: string): string {
    let a;
    let rgb: any[] = color.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+),?([^,\s)]+)?/i);
...
export function detectWebcam(): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      ...
      mediaDevices.enumerateDevices().then((devices: MediaDeviceInfo[]) => {
